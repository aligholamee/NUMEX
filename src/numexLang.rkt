...
;; Problem 1
(define (racketlist->numexlist xs) (if (null? xs) (munit) ;; in case the result is null -> return munit (numex null value)
                                       (apair (if (integer? (car xs)) (int (car xs))
                                                  (boolean? (car xs)) (bool (car xs))
